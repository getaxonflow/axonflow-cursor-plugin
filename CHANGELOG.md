...
- **`audit_tool_call` now sends `caller_name` to identify the calling plugin, dual-sent with the legacy `tool_type` for the deprecation window** (#83; axonflow-enterprise#2912, sub-issue of epic #2905). The `tool_type` field was misleadingly named — it identified WHICH CLIENT made the call, not a tool type. The platform side (axonflow-enterprise#2953, shipped in platform 9.11.0) adds the correctly-named `caller_name` field and resolves `caller_name → tool_type → default`, writing only `caller_name` on new rows; `tool_type` remains a deprecated legacy fallback that pre-#2953 platforms still honor. Both send sites — `scripts/pre-tool-check.sh` and `scripts/post-tool-audit.sh` — now send **both** `caller_name: "cursor"` and `tool_type: "cursor"`. Dual-send is exact on a 9.11.0+ platform (`caller_name` wins) **and** status-quo on any pre-#2953 platform (the legacy `tool_type` still attributes the row) — critical because a pre-#2953 agent drops the unknown `caller_name` and defaults an absent `tool_type` to `"claude_code"`, misattributing every cursor row. `tool_type` will be dropped once the platform floor includes #2953. This matches the transition pattern across the claude/codex/openclaw siblings, and the aligned envelope-surface e2e + manual runbook now dual-send too. The host-CLI shim (`tests/host-cli-shim/run.sh`, CI-gated) now asserts the on-the-wire `audit_tool_call` payload carries both keys, so a silent revert fails CI rather than passing on request-count alone.
