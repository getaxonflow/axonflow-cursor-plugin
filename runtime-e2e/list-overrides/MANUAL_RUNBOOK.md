...
4. **Send the prompt verbatim:**
   ...
   > Use the `list_overrides` MCP tool from the axonflow MCP server with arguments: (no arguments). The platform will respond — expected outcome: {overrides: [...], count: N} (empty in community mode). Output exactly `SMOKE_RESULT: ` followed by a one-line JSON summary of what happened.
